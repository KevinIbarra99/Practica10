            ...
            if(!correo.isNullOrBlank()){
                auth.sendPasswordResetEmail(correo)
                    .addOnCompleteListener{ task ->
                        if(task.isSuccessful){Toast.makeText(this, "Se envio un correo a ${correo}!", Toast.LENGTH_SHORT).show()}
                        else{Toast.makeText(this, "Error al enviar el correo!", Toast.LENGTH_SHORT).show()}
